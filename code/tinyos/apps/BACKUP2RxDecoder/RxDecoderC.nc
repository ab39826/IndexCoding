...
implementation {
	...
	uint8_t interf_vec[I] = {1, 3, 5, 7}; // I interference dimensions, pass to mote instead of harcode?
	
	int ant_vec[A] = {2,4,6}; //antidote dimensions, pass to mote instead of hardcode?
	...
	event message_t* Receive.receive(message_t* bufPtr, void* payload, uint8_t len) {
		...
		if (len != sizeof(decoded_msg_t)) {
			...
			decoded_msg_t* rcm = (decoded_msg_t*)payload;
				...
				//????
				if(rcm->crow == 255){ //cannot be <0 or >N?
					for (i = 0; i<A; ++i){
						for(j = 0; j<PIECES; ++j){
							known_data[i][j] = rcm->V_coeff[i]; //pass to mote instead of hardcode, is this only called once right now??
						}
					}
			...
			//success = null_vec(V_null_tr,I, current_row +1, S,(.0001),W,rv1); //ORIGINAL
			success = null_vec(V_null_tr,N, current_row +1, S,(.0001),W,rv1); //MAKE SVD WORK???
			//success = null_vec(V_null_tr,I, current_row +1, S,(.001),W,rv1);
				
			//success  = svd(V_null_tr, I, (current_row +1), S, W, rv1); // sanity check
			
					
			//DEBUG, go straight to end
			//decode_status= FALSE;	
			if (success) {
