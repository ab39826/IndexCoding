...
implementation {
  ...
  //components new AMReceiverC(AM_ACK_MSG) as RadioReceiverC; //FOR RECEVIER TO RECEIVER TEST
	...
	//components new AMReceiverC(AM_ACK_MSG) as RadioReceiverC; //whats in () doesnt actually matter?
  components new AMReceiverC(AM_SYMBOL_MSG) as RadioReceiverC; //whats in () doesnt actually matter?
  components new AMSenderC(AM_ACK_MSG) as RadioSenderC; 
