function [TOTAL_TRANSMISSIONS] = index_coding_sim2(K, eps_vec, verbose)
%index_coding_sim Run index coding simulation 2
% Step 1: TDMA
% Step 2: Generate m = minrank symbols
% Step 3: Send all m symbols
% Step 4: Decode and update
% Step 5: Go to Step 2
% Inputs:
%  K: number of nodes
...
%         [V U] = alignment('mixed', J, nan, 1e-4, 100, 1);
        [V U] = alignment('greedy', J, nan, 1e-4, 100, 0);
        m = size(V,1);
%         mg = size(Vg,1);
...
%             sym_vals(i) = tx_symbols(L+i).val;
        ...
        for i = 1:length(tx_symbols)
            tx_symbols(i).v_coeffs = [tx_symbols(i).v_coeffs zeros(1,m)];
